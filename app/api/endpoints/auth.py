...
@router.post("/login", response_model=user_schemas.Token, status_code=status.HTTP_200_OK)
def user_login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    db_user = crud_user.get_user_by_email(db, email=form_data.username)
    if not db_user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")
    user_password = security.verify_password(form_data.password, db_user.hashed_password)
    ...
    db_refresh_token = crud_user.create_refresh_token_entry(db, user_id=db_user.id, token=refresh_token,
                                                            expires_at=expired_at)
    return {"access_token": access_token, "refresh_token": refresh_token, "token_type": "bearer"}


@router.post("/logout", status_code=status.HTTP_204_NO_CONTENT)
def user_logout(token_data: user_schemas.TokenExchange, db: Session = Depends(get_db),
...
@router.post("/refresh", response_model=user_schemas.Token, status_code=status.HTTP_200_OK)
def refresh_token(token_data: user_schemas.TokenExchange, db: Session = Depends(get_db)):
    db_token = crud_user.get_refresh_token(db, token=token_data.token)
    if not db_token:
    ...
    new_expires_at = datetime.now(timezone.utc) + timedelta(days=security.REFRESH_TOKEN_EXPIRE_DAYS)
...
def password_reset_confirm(data: user_schemas.PasswordResetConfirm, db: Session = Depends(get_db)):
    user =  crud_user.reset_password_with_token(db, token_value=data.token, new_password=data.new_password)
    if user is None:
