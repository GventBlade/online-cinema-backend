...
def create_favorite(
    ...
    return interactions_crud.toggle_favorite(
...
def rate_movie(
    ...
    get_movie(db, movie_id)
    return interactions_crud.rate_movie(
        db=db, movie_id=movie_id, user_id=current_user.id, score=score_in.score
    )
...
def create_comment(
    ...
    get_movie(db, movie_id)
    return interactions_crud.create_comment(
        db=db, user_id=current_user.id, movie_id=movie_id, comment_in=comment_in
    )
...
@router.get(
    "/{movie_id}/rating", response_model=movies_schema.MovieAverageRatingResponse
)
